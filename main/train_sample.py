...
# from utils.matplotlib_helpers import set_default_paras
# from utils.prepare_training import prepare_training
import logging
import argparse
from glob import glob
import yaml
from main.utils.wandb_helpers import merge_configs_recursive
from main.systems.aldp.simulation.initialize_simulation import get_forces
...
if __name__ == "__main__":
    ...
    # AL_manager._train_epoch_by_example()
    for i in range(51):
        AL_manager.train_epoch()
    print("Device name before: " + str(torch.cuda.get_device_name(torch.cuda.current_device())))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print("Device after assignment: ", device)

    cg_configs = AL_manager.AL_dataset.get_current_s().cuda()
    print("cg configs:", cg_configs)
    print("cg configs shape:", cg_configs.shape)
    print("first 5 cg configs:", cg_configs[:5,:])
    print("first 5 cg configs:", cg_configs[:5,:].shape)
    sampled = AL_manager.AL_dataset.cond_flow.sample(5, context=cg_configs[:5,:])
    # sampled = AL_manager.AL_dataset.cond_flow.sample(120000, context=cg_configs)
    # sampled = AL_manager.AL_dataset.cond_flow.q0.sample(5)
    print(sampled)
    print("\nsampled shape")
    print(sampled[0].shape)

    single_cg = cg_configs[0]
    repeated_single_cg = single_cg.repeat(10, 1)
    print("single cg")
    print(single_cg)
    print(single_cg.shape)
    print("repeated single cg 10 times")
    print(repeated_single_cg)
    print(repeated_single_cg.shape)

    cg_single_sampled = AL_manager.AL_dataset.cond_flow.sample(10, repeated_single_cg)
    print("sampled from repeated cg")
    print(cg_single_sampled)
    print(cg_single_sampled[0].shape)
    internal = cg_single_sampled[0]

    cg_indices = AL_manager.system.CG_indices
    print("cg indices:", cg_indices)

    internal = torch.cat([
        internal[:cg_indices[0]],
        single_cg[0],
        internal[cg_indices[0]:]
    ])
    internal = torch.cat([
        internal[:cg_indices[1]],
        single_cg[1],
        internal[cg_indices[1]:]
    ])
    print("full internal")
    ...
    current_xs = AL_manager.AL_dataset.get_current_xs_int()
    print("\ncurrent xs shape")
    print(current_xs.shape)
    print(current_xs)

    cart_xs = AL_manager.system.internal_to_cartesian(current_xs)
    # cart_sampled = AL_manager.system.internal_to_cartesian(sampled[0])
    print("\ncartesian xs")
    # print(cart_sampled.shape)
    # print(cart_sampled)
    print(cart_xs.shape)
    print(cart_xs)
